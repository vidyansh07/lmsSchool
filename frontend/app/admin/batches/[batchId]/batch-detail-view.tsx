...
export function RosterPanel({ batch, onChanged }: { batch: BatchDetail; onChanged: () => void }) {
  const { can } = useAuth();
...
export function BatchDetailView({ batchId }: { batchId: string }) {
  ...
  // A single source of truth for loading the batch: the initial fetch on
  // mount and every refresh after a mutation call the same function, so
  // there is exactly one place that knows how to fetch it.
