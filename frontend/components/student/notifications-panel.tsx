    ...
    <div className="space-y-3">
      <p aria-live="polite" className="flex items-center gap-1.5 text-sm">
        ...
        {unreadCount > 0 ? (
          ...
          <span className="text-ink-muted">You&apos;re all caught up.</span>
        )}
      </p>
          ...
          {notifications.map((item) => {
            const body = (
              <>
                ...
                <p className="text-xs text-ink-muted">{formatRelative(item.created_at)}</p>
              </>
            );
            return (
              <li key={item.id} className="rounded-card border border-line p-2.5">
                {item.link_path ? (
                  <Link href={item.link_path} className="block hover:text-action">
                    {body}
                  </Link>
                ) : (
                  body
      ...
      <Link href="/notifications" className="inline-block text-sm underline hover:text-ink">
        All notifications
      </Link>
    </div>
  );
